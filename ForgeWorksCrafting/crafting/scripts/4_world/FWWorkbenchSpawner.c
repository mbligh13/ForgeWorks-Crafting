...
// Path: scripts/4_world/FWWorkbenchSpawner.c

class FWWorkbenchSpawner
{
    // Call this from your mission init to spawn workbenches at defined positions
    static void SpawnAtPosition(vector position, vector orientation = "0 0 0", string workbenchType = "FWCraftingWorkbench")
    {
        if (!GetGame().IsServer())
            return;
        ...
        if (workbench)
        {
            ...
            Print("[ForgeWorks] Spawned " + workbenchType + " at " + position.ToString());
        }
    }
    ...
    static void SpawnNearVanillaWorkbench(string vanillaWorkbenchClass = "Land_Repair_Center")
    {
        ...
        // Find all vanilla workbenches and place ForgeWorks workbench nearby
    ...
    static void SpawnFromString(string coordString)
    {
        ...
        if (parts.Count() < 3)
        {
            Print("[ForgeWorks] Invalid coordinate format. Use: x,y,z or x,y,z,yaw");
            return;
        }
...
// ============================================
class FWAdminSpawnWorkbench
{
    static void Execute(PlayerBase player, string type = "FWCraftingWorkbench")
    {
        if (!player)
            return;
        ...
        FWWorkbenchSpawner.SpawnAtPosition(spawnPos, ori, type);
    }
}
